...
const GanttChart = ({ projects }: GanttChartProps) => {
  ...
  const getBarColor = (project: Project) => {
    ...
    switch (priority) {
      case 'high': return 'bg-priority-high/70';
      case 'medium': return 'bg-priority-medium/70';
      case 'low': return 'bg-priority-low/70';
            ...
            return (
              <div key={project.id} className="flex group">
                ...
                <div className="w-3/4 relative h-8">
                  ...
                  {/* Deadline marker */}
                  <div 
                    className="absolute top-0 bottom-0 w-px bg-black dark:bg-white z-20"
                    style={{ left: getBarPosition(project.deadline) }}
                  >
                    <div className="w-2 h-2 rounded-full bg-black dark:bg-white absolute top-1/2 -translate-x-1/2 -translate-y-1/2" />
                  </div>
                </div>
              </div>
            );
